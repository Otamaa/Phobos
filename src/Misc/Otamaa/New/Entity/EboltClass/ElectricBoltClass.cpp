...
void ElectricBoltClass::Plot_Bolt(CoordStruct& start, CoordStruct& end)
{
	...
	constexpr auto Distance = [](const CoordStruct& coord1, const CoordStruct& coord2)
	{
		CoordStruct coord;
		coord = coord1 - coord2;
	...
	constexpr auto Sim_Random_Pick = [](int a, int b)
	{
	...
	if (distance)
	{
		...
		int dist_a = (102 * distance / 256);

		/**
		 *  Max distance from line center, with "Deviation" as delta.
		 */
		int desired_deviation = 23;
		int line_deviation = (int)((desired_deviation * Deviation) * distance / 256);

		while (true)
		{

			while (distance > (256 / 4) && plot_index < ebolt_plots.Length())
			{
				...
				if (init_deviation_values)
				{
					...
					{
						deviation_values[i] = (int)(Math::sin((double)Sim_Random_Pick(0, 256) * Math::Pi / (double)(i + 7)) * (double)line_deviation);
					}

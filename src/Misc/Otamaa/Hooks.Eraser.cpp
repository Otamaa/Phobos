...
DEFINE_HOOK(0x416619, AircraftClass_ReceiveDamage_Eraser, 0x6)
{
	GET_STACK(WarheadTypeClass*, pWarhead, 0x24);

	auto const pWHExt = WarheadTypeExt::ExtMap.Find(pWarhead);
	if (pWHExt->AnotherData.Eraser.Get())
	{
		R->AL(false);
		return 0x41669A;
	}
...
DEFINE_HOOK(0x70222E, TechnoClass_ReceiveDamage_Eraser, 0xA)
{
	...
	if (pWHExt->AnotherData.Eraser.Get())
	{
		...
		if (auto pAnimType = pWHExt->AnotherData.Eraser_Anim.Get())
		{
...
DEFINE_HOOK(0x442651, BuildingClass_ReceiveDamage_Eraser, 0xB)
{
...
DEFINE_HOOK(0x5181B5, InfantryClass_ReceiveDamage_Eraser, 0xA)
{
	GET_STACK(WarheadTypeClass*, pWarhead, 0xDC);
	auto const pWHExt = WarheadTypeExt::ExtMap.Find(pWarhead);
	return pWHExt->AnotherData.Eraser.Get() ? 0x518619 : 0x0;
}

DEFINE_HOOK(0x737D9C, UnitClass_ReceiveDamage_Eraser, 0x6)
{

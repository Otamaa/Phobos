...
DEFINE_HOOK(0x471C0C, CaptureManagerClass_Overload_ParticleSys, 0x6)
{
	...
	R->ECX(pExt->AnotherData.Overload_ParticleSys.Get(pRules->DefaultSparkSystem));
...
DEFINE_HOOK(0x4721E6, CaptureManagerClass_DrawLinkToVictim, 0x6)
{
	...
	if (pExt->AnotherData.Draw_MindControlLink.Get())
	{
...
void __fastcall CaptureManagerClass_Overload_AI(CaptureManagerClass* pThis, void* _)
{
	...
	if (pThis->InfiniteMindControl)
	{
		...
		{
			auto const& OverloadCount = pOwnerTypeExt->AnotherData.Overload_Count.GetElements(pRules->OverloadCount);
			...
			auto const& nOverloadfr = pOwnerTypeExt->AnotherData.Overload_Frames.GetElements(pRules->OverloadFrames);
			pThis->OverloadDamageDelay = FixIdx(nOverloadfr, nCurIdx);

			auto const& nOverloadDmg = pOwnerTypeExt->AnotherData.Overload_Damage.GetElements(pRules->OverloadDamage);
			...
			else
			{
				...
				{
					VocClass::PlayAt(pOwnerTypeExt->AnotherData.Overload_DeathSound.Get(pRules->MasterMindOverloadDeathSound), pOwner->Location, 0);
					pThis->OverloadDeathSoundPlayed = true;
				}

				if (auto const pParticle = pOwnerTypeExt->AnotherData.Overload_ParticleSys.Get(pRules->DefaultSparkSystem))
				{
					for (int i = pOwnerTypeExt->AnotherData.Overload_ParticleSysCount.Get(5); i > 0; --i)
					{

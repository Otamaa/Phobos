...
namespace _ReplaceMakePath
{
	...
	{
		CRT::_makepath(arg1, arg2, arg3, arg4, TheaterTypeClass::GetExtension(CURRENT_THEATER));
	}

	void __cdecl Exec2(char* arg1, const char* arg2, const char* arg3, const char* arg4, const char* arg5)
	{
		char nBuffer[256];
		_snprintf_s(nBuffer, _TRUNCATE, ".%s", TheaterTypeClass::GetCharExtension(CURRENT_THEATER));
		CRT::_makepath(arg1, arg2, arg3, arg4, nBuffer);
	}
}
...
DEFINE_HOOK(0x5349E3, ScenarioClass_InitTheater_TheaterType_ControlFileName, 0x6)
{
	...
	ScenarioExt::Global()->MyCurrentTheater = TheaterTypeClass::As_Pointer_(nType);
...
DEFINE_HOOK(0x534A3C, ScenarioClass_initTheater_TheaterType_DebugLog, 0x6)
{
	Log::Exec(__FUNCTION__, CURRENT_THEATER);
	GET(TheaterType, nType, EDI);
	//VeinholeMonsterClass::TheaterInit(nType);
...
namespace _fetchColor
{
	ColorStruct __fastcall Exec(ColorStruct& nReturn, ColorStruct& nColor, float nval)
	{
		...
		Log::Exec(__FUNCTION__, CURRENT_THEATER);
		return ColorStruct::AdjustBrightness(nColor, TheaterTypeClass::GetLowRadarBrightness(CURRENT_THEATER));
	}
}
...
{
	bool __fastcall Exec(INIClass* nIni, char* section, char* entry, TheaterType value)
	{
		Debug::Log(__FUNCTION__" Exec sec[%x] size[%d]  \n", nIni, sizeof(nIni));
		return nIni->WriteString("Map", "Theater", TheaterTypeClass::GetIdentifier(value));
	}
}
...
DEFINE_HOOK(0x627699, TheaterTypeClass_ProcessOtherPalettes_Process, 0x6)
{
	...
	char nBuffer[256];
	_snprintf_s(nBuffer, _TRUNCATE, "%s%s.PAL", pOriginalName, TheaterTypeClass::GetCharExtension(CURRENT_THEATER));
	CRT::strupr(nBuffer);// Uppercase all !

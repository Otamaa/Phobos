...
bool IonBlastExt::DoAffects(IonBlastClass* pIon)
{
	...
	if (auto pData = IonBlastExt::Find(pIon))
	{
		...
		{
			pWarhead = pWarheadExt->IonWH.Get(pWarhead);
			pBlastAnim = pWarheadExt->IonBlast.Get(pBlastAnim);
			pBeamAnim = pWarheadExt->IonBeam.Get(pBeamAnim);
			nDamage = pWarheadExt->IonDamage.Get(nDamage);
	...
	if (pBeamAnim)
		if (auto pBeam = GameCreate<AnimClass>(pBeamAnim, nAnimLoc))
			AnimExt::SetAnimOwnerHouseKind(pBeam, pHouseOwner, nullptr, false);

	if (pBlastAnim)
		if (auto pBlast = GameCreate<AnimClass>(pBlastAnim, nAnimLoc))
			AnimExt::SetAnimOwnerHouseKind(pBlast, pHouseOwner, nullptr, false);
